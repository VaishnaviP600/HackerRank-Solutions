...
# Create an empty canvas of underscores
for ((i = 0; i < rows; i++)); do
    canvas[i]=$(printf '%*s' "$cols" '' | tr ' ' '_')
done

# Function to draw tree recursively
draw_tree() {
    local x=$1
    local y=$2
    local h=$3
    ...
    for ((i = 0; i < h; i++)); do
        row=$((x - i))
        line="${canvas[$row]}"
        canvas[$row]="${line:0:$y}1${line:$((y + 1))}"
    done
    ...
    for ((i = 1; i <= h; i++)); do
        ...
        col=$((y - i))
        line="${canvas[$row]}"
        canvas[$row]="${line:0:$col}1${line:$((col + 1))}"
    ...
    # Recursive calls for smaller Ys
    draw_tree $((x - 2 * h)) $((y - h)) $((h / 2)) $((n - 1))
    draw_tree $((x - 2 * h)) $((y + h)) $((h / 2)) $((n - 1))
}

# Start drawing from bottom-center
draw_tree 62 49 16 "$N"
